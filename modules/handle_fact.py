...
async def handle_fact(context):
    ...
    facts = db.execute(
        f'SELECT fact FROM facts where server_id = "{event.message.get_server()[1]}"'
    ).fetchall()
    (last_fact, chance) = db.execute(
        f'SELECT last_fact, fact_chance FROM server_settings where server_id = "{event.message.get_server()[1]}"'
    ).fetchone()
    ...
    if last_fact is None:
        ...
        db.execute(
            f'INSERT INTO server_settings (server_id) VALUES ("{event.message.get_server()[1]}")'
        )
        db.connection.commit()
    else:
        print(
            f"Last fact index: {last_fact}, Chance: {chance}, Random Chance: {random_chance}"
        )
    ...
    if random_chance <= chance:
        ...
        last_fact += 1
        db.execute(
            f'UPDATE server_settings SET last_fact = {last_fact} WHERE server_id = "{event.message.get_server()[1]}"'
        )
        db.connection.commit()
