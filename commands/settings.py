...
async def settings(context):
    ...
    if subcommand in subcommand_mapping:
        ...
        await event.message.channel.send(
            "Invalid use of command, see embed for proper usage.", embeds=[helper_embed]
        )
    ...
    if mode == "set":
        if len(args) < 2:
            await event.message.reply(
                "Please provide a welcome message. Use {user} to mention the new member."
            )
            return
        ...
        db.execute(
            "UPDATE server_settings SET welcome_message = ? where server_id = ?;",
            [new_message, server_id],
        )
        db.connection.commit()
        await event.message.reply("Welcome message updated successfully!")

    elif mode == "view":
        (current_message,) = db.execute(
            "select welcome_message from server_settings where server_id = ?",
            [server_id],
        ).fetchone()
        ...
        if current_message:
            ...
            await event.message.reply(
                "No welcome message has been set for this server yet."
            )
...
async def settings_welcome_channel(
    ...
    if not args:
        await event.message.reply(
            "Please specify a mode: `set {channel}`, `view`, or `disable`"
        )
        return
    ...
    if mode == "set":
        ...
        [channel_id] = db.execute(
            "select welcome_channel_id from server_settings where server_id = ?",
            [server_id],
        ).fetchone()
        ...
        await event.message.reply(
            "Welcome channel disable functionality not yet implemented."
        )

    else:
        await event.message.reply(
            "Invalid mode. Use `set {channel}`, `view`, or `disable`."
        )


async def settings_welcome_image(
    event: stoat.MessageCreateEvent,
    db: sqlite3.Cursor,
    client: stoat.Client,
    args: list[str],
    ...
    if not args:
        await event.message.reply(
            "Please specify a mode: `set {image_url}`, `view`, or `remove`"
        )
        return
    ...
    if mode == "set":
        ...
        print(
            f"[Attachment details] Content type: {asset.content_type}, tags: {asset.tag}, metadata: {asset.metadata}"
        )
        ...
        if not allowed_datatypes.__contains__(asset.content_type):
            await event.message.reply(
                f"Incorrect type, these are the allowed image types: {allowed_datatypes.__str__()}"
            )
        ...
        db.execute(
            "update server_settings set welcome_image_blob = ? where server_id = ?",
            [byte_data, server_id],
        )
        db.connection.commit()
        ...
        (welcome_image_blob,) = db.execute(
            "select welcome_image_blob from server_settings where server_id = ?;",
            [server_id],
        ).fetchone()
        if welcome_image_blob is None:
            ...
            return
        await event.message.reply(
            "Here's the current welcome image!",
            attachments=[cast(stoat.ResolvableResource, welcome_image_blob)],
        )

    elif mode == "remove":
        # Placeholder for removing welcome image
        # TODO - Implement remove join image
        await event.message.reply(
            "Welcome image remove functionality not yet implemented."
        )

    else:
        await event.message.reply(
            "Invalid mode. Use `set {image_url}`, `view`, or `remove`."
        )


async def settings_fact_chance(
    event: stoat.MessageCreateEvent,
    db: sqlite3.Cursor,
    client: stoat.Client,
    args: list[str],
