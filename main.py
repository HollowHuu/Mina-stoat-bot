...
async def on_member_join(event: stoat.ServerMemberJoinEvent, /):
    ...
    (welcome_message, welcome_channel_id, welcome_image_blob) = db.execute('SELECT welcome_message, welcome_channel_id, welcome_image_blob FROM server_settings WHERE server_id = ?;', (server.id,)).fetchone()

    print(welcome_channel_id)

    welcome_message = welcome_message.replace('{user}', event.member.mention)
    welcome_channel = next((c for c in server.channels if c.id == welcome_channel_id), None)

    if welcome_channel is None:
        print(f"Couldn't find welcome channel with ID: {welcome_channel_id}")
        return

    await welcome_channel.send(welcome_message, attachments=[
        cast(stoat.ResolvableResource, welcome_image_blob)
    ])
